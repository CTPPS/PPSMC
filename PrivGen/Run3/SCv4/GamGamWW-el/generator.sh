...
if [ ! -d "${5}/${3}" ];
then
    xrdfs eoscms.cern.ch mkdir -p ${5}/${3}
fi
# folders for split and single files:
xrdfs eoscms.cern.ch mkdir -p ${5}/${3}/split
# run
./superchic < ${label}.card > ${label}.txt
mv evrecs/evrec${label}.dat ${label}.lhe
# remove these lines to use xfer:
xrdcp -fs ${label}.lhe ${4}/${5}/$3/split/${label}.lhe
xrdcp -fs ${label}.txt ${4}/${5}/$3/split/${label}.txt
xrdcp -fs ${label}.card ${4}/${5}/$3/split/${label}.card
# clean working node
cd ../../
rm -rf superchic4.13* 

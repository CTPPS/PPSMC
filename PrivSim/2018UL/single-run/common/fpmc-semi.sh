...
then
    xrdfs eoscms.cern.ch mkdir -p ${eosarea}/${tag}/LHE/split
fi
# run:
...
xrdcp -f ${label}.lhe.xz ${eosroot}/${eosarea}/${tag}/LHE/split/${label}.lhe.xz
xrdcp -f ${label}.txt ${eosroot}/${eosarea}/${tag}/LHE/split/${label}.txt
xrdcp -f ${label}.card ${eosroot}/${eosarea}/${tag}/LHE/split/${label}.card
# clean working node:
cd ../..
rm -rf fpmc/
rm -rf `whoami`.cc

...
C
C Write start of event in the lhe file
ccc SELECTION ON THE INVARIANT MASS OF PHOTON PAIRS
      IF(PHEP(5,3).LT.300)GO TO 999
ccc
      WRITE(45,40)
      ...
      DO 510 I=1,NHEP
        IF(IDHEP(I).EQ.2212.AND.JMOHEP(1,I).EQ.1)IPR1=I
        IF(IDHEP(I).EQ.2212.AND.JMOHEP(1,I).EQ.2)IPR2=I
cc fix1 Miguel, next line delete ID=0 in STATUS=103 in count
        IF(IDHEP(I).EQ.0.AND.ISTHEP(I).EQ.103) GO TO 510
cc end fix1     
        NUP = NUP + 1
  510 CONTINUE
      ...
      DO 511 I=1,NHEP
        ...
        IF(IDHEP(I).EQ.2212.AND.JMOHEP(1,I).EQ.2)IPR2=I
c fix2 Miguel    
        IF(IDHEP(I).EQ.2212.AND.JMOHEP(1,I).EQ.0) THEN
         IF(JDAHEP(1,I).GT.0)JDAHEP(1,I)=JDAHEP(1,I)-1
         IF(JDAHEP(2,I).GT.0)JDAHEP(2,I)=JDAHEP(2,I)-1
        ENDIF
cc delete lines with id==0 and status 103 
        IF(IDHEP(I).EQ.0.AND.ISTHEP(I).EQ.103) THEN
          DL=1
          GO TO 511
        ENDIF 
    
        IF(ISTHEP(I).EQ.121.OR.ISTHEP(I).EQ.122) THEN
          JDAHEP(2,I)=JDAHEP(1,I)
          JDAHEP(1,I)=JMOHEP(1,I)
          IF(ISTHEP(I).EQ.121) JMOHEP(1,I)=JMOHEP(1,I)-6
          IF(ISTHEP(I).EQ.122) JMOHEP(1,I)=JMOHEP(1,I)-4
          IF(IPRO.EQ.61)JMOHEP(2,I)=JMOHEP(1,I)+2
          IF(IPRO.EQ.11)JMOHEP(2,I)=JMOHEP(1,I)+2
          JMOHEP(2,I)=I
          CORRHEP=.TRUE.
        ENDIF
        IF(CORRHEP) THEN
          JMOHEP(1,I)=JMOHEP(1,I)-DL
          JMOHEP(2,I)=JMOHEP(2,I)-DL
          IF(JDAHEP(1,I).GT.0)JDAHEP(1,I)=JDAHEP(1,I)-DL
          IF(JDAHEP(2,I).GT.0)JDAHEP(2,I)=JDAHEP(2,I)-DL
        ENDIF
cc end fix2 miguel

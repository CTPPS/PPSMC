...
C FILTER: Gustavo Silveira GGS
      ILEP  = 0
      IQURK = 0
      IJET  = 0

      PtCutJet =  0   ! cut on pT of Jets
      PtCutMu  =  20   ! cut on pT of Muons
      PtCutEl  =  20   ! cut on pT of Electrons
      PtCutTau =  20   ! cut on pT of Tauons 
...
C     qqelnu = 122, qqmunu = 122, qqqq = 044, leps = 200
      IF (ILEP.EQ.2.AND.IQURK.EQ.0.AND.IJET.EQ.0) THEN
        CONTINUE
      ELSE
        GOTO 542
      ENDIF

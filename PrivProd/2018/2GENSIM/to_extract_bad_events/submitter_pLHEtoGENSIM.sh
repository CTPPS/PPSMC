...
do
#   input="$1"_pLHE_"$i".root # list of pLHE files
   input="$1"_pLHE_0.root # single pLHE file
   cfginput="$1"_"$step"_cfg_"$i".py
   shinput="$1"_"$i".sh
   subinput="$1"_"$i".sub
   output="$1"_"$step"_"$i".root
   first=$(( i*1000+1 ))
   evts=$(( 1000-`awk '{print (int($1/1000)+1)*1000}' ../extract_bad_events.txt | sort -g | grep -cw $(( (i+1)*1000 ))` ))
   ...
   sed -i "s/xevt/$evts/g" 0cfg/$cfginput
